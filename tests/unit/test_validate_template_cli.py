...
class TestValidateTemplatePreFlight(unittest.TestCase):
    ...
    def test_validate_detects_missing_keys(self):
        ...
        report = {
            "resolved": [],
            "missing": ["@endereco", "@telefone"],
            "none_values": [],
            "formulas": []
        }
        with patch.object(self.service, '_validate_keys', return_value=report):
            result = self.service.validate_template_keys(
                str(self.test_dir / 'template.docx'),
                str(data_file),
                'docx'
            )

        self.assertIn('@endereco', result["missing"])
        self.assertIn('@telefone', result["missing"])

    def test_validate_passes_when_all_keys_present(self):
        """Should return empty missing list when all template keys are in data."""
        data_file = self.test_dir / 'data.md'
        data_file.write_text(
            '@nomeCliente: João\n@cpf: 12345\n',
            encoding='utf-8'
        ...
        report = {"resolved": [{"key": "@nomecliente", "value": "João"}], "missing": [], "none_values": [], "formulas": []}
        with patch.object(self.service, '_validate_keys', return_value=report):
            result = self.service.validate_template_keys(
                str(self.test_dir / 'template.docx'),
                str(data_file),
