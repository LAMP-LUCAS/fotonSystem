...
class WebViewFormAdapter(FormInterfacePort):
    """Adapter que abre o formulário em uma janela WebView nativa (pywebview)."""

    def open_form(self, initial_content: str, save_callback: Callable[[str], bool]) -> bool:
        """Abre janela WebView para preenchimento interativo do formulário."""
        try:
            import webview
            from foton_system.interfaces.webview_bridge import WebViewBridge

            html_path: Path = Path(__file__).resolve().parents[4] / "interfaces" / "fotonInfoInterface.html"
            ...
            api: WebViewBridge = WebViewBridge(initial_content, save_callback)
