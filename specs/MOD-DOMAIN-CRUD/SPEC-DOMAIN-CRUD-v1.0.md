...
4. **UX sem orientação** — Menus longos sem breadcrumbs, sem busca global, sem feedback de progresso. O usuário se perde e acha que o sistema travou.
...
### UX Modernizada (refatoração)

Divisão do `menus.py` monolítico em submódulos. ProgressTracker para operações batch. Erros com sugestões de ação. Menu reestruturado em subgrupos. Busca global com atalho `g`. Paginação em MCP tools.
...
- **RULE-DOMAIN-4.1:** `menus.py` (54KB) deve ser dividido em submódulos: `menus_clients.py`, `menus_finance.py`, `menus_docs.py`, `menus_config.py`. `menus.py` mantém apenas o dispatch principal.
- **RULE-DOMAIN-4.2:** `ProgressTracker` deve exibir feedback visual para operações batch no formato `"[3/10] Processando CLIENTE..."` com `advance(item)` e `finish()`.
- **RULE-DOMAIN-4.3:** Erros capturados na TUI devem exibir sugestões de ação contextualizadas por tipo (ex: `FileNotFoundError` → "Verifique settings.json", `PermissionError` → "Feche o Excel e tente novamente").
- **RULE-DOMAIN-4.4:** Menu de clientes deve ser reestruturado em subgrupos visuais: "--- Cadastro ---", "--- Manutenção ---", "--- Serviços ---", "--- Perigo ---". Ações destrutivas agrupadas sob "Perigo".
- **RULE-DOMAIN-4.5:** Atalho `g` no menu principal deve disparar busca global (`global_search`) por alias, nome, código ou NIF. Resultados numerados com drill-down para ficha do cliente.
- **RULE-DOMAIN-4.6:** `parse_command()` deve interpretar atalhos no input principal: `h` (ajuda), `00` (home/início), `q` (sair), texto livre (busca).
- **RULE-DOMAIN-4.7:** MCP `listar_clientes` deve aceitar parâmetros opcionais `pagina` (int, default 1) e `itens_por_pagina` (int, default 20) — backward compatibility mantida.
- **RULE-DOMAIN-4.8:** Confirmação padronizada (S/N) em todas as ações destrutivas via `confirm_action()`, com variação visual `dangerous=True` para operações irreversíveis.
...
- [ ] RULE-DOMAIN-2.1 a 2.7: 4 novas MCP tools registradas, backup `.bak` antes de delete, POP auditado
- [ ] RULE-DOMAIN-3.1 a 3.5: Pipeline unificado com dry-run, SyncReport JSON, aliases legados funcionando
- [ ] RULE-DOMAIN-4.1 a 4.4: menus.py dividido, ProgressTracker funcional, erros com sugestões, menu com subgrupos
- [ ] RULE-DOMAIN-4.5 a 4.8: Busca global, parse_command, paginação MCP, confirmação padronizada
...
- **Menus legados mantidos** durante transição — a divisão de `menus.py` é feita em etapas para evitar quebra.
...
- Specs base: `SPEC-CLIENTES-v1.0` (Value Objects, CRUD base), `SPEC-UX-v1.0` (breadcrumbs, paginação, confirmações), `SPEC-SYNC-v1.0` (direções de sync)
